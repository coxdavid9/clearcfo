...
export async function PATCH(request: Request) {
  try {
    ...
    const status = message === "Unauthorized" ? 401 : message === "Business not found." ? 404 : message.startsWith("upgrade_required:") ? 403 : 400;
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: message.startsWith("upgrade_required:") ? "upgrade_required" : message, message: message.startsWith("upgrade_required:") ? message.replace(/^upgrade_required:\s*/, "") : undefined }, { status: message === "Unauthorized" ? 401 : message.startsWith("upgrade_required:") ? 403 : 400 });
  }
}
