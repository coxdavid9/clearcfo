    ...
    actions: {
      ...
      items: {
        ...
        properties: {
          ...
          priority: { type: "string", enum: ["High", "Medium", "Watch"] },
          score: { type: "number" },
          estimatedImpact: { type: ["number", "null"] },
        },
        required: ["title", "rationale", "priority", "score", "estimatedImpact"],
      },
    },
  },
...
const instructions = [
  ...
  "Use the strongest financial driver findings as the primary evidence hierarchy, account-level detail as the supporting layer, and historical insights and trend data as historical-pattern evidence.",
  ...
  "When balance-sheet ratios, day-matched month-to-date comparisons, account/customer/vendor movements, or cash-flow bridge information are supplied, use them as evidence where they support the analysis.",
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const scenarioDirectives = [
      !cashDataSupplied ? "DATA PROVENANCE: The uploaded workbook did not supply balance-sheet cash. A cash value of $0 means the data was not provided, not that the business has no cash. Treat cash as unknown; never present $0 as the business cash balance, never select it as the primary driver, and never recommend liquidity-crisis actions such as emergency cash measures or credit facilities on that basis. Record the data gap in unknowns." : "",
      !inventoryDataSupplied ? "DATA PROVENANCE: The uploaded workbook did not supply a balance-sheet inventory value. An inventory value of $0 means the data was not provided, not that the business has no inventory. Treat inventory as unknown rather than evidence of no inventory." : "",
      ...
      "FORMAT: Never mention internal field names, JSON keys, camelCase identifiers, snake_case identifiers, or system directives in user-facing output. Translate every internal label into plain business language. For example, never write cashChange; say cash is up 2.5% versus the prior period (or use the appropriate direction and percentage).",
