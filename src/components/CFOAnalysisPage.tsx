...
function normalizeAnalysis(analysis: AIAnalysis): AIAnalysis {
  ...
  return {
    ...
    primaryDriver: text(analysis.primaryDriver),
    whyItMatters: text(analysis.whyItMatters),
    managementQuestion: text(analysis.managementQuestion),
    recommendedAction: text(analysis.recommendedAction),
    evidence: analysis.evidence.map(text),
      ...
      try {
        ...
        const response = await fetch("/api/cfo-analysis", {
          ...
          body: JSON.stringify({
            ...
            detectedIssues: input.alerts,
            financialDrivers: input.drivers,
            driverRelationships: input.relationships,
            detailDrivers: input.detailDrivers,
            currentRecommendation: input.recommendation,
            businessHealth: input.health,
            analysisConfidence: input.confidence,
            recentRevenueTrend: input.trend.slice(-12),
            periods: input.periods.slice(-12),
            multiPeriodInsights: input.trendInsights,
            knownUnknowns: input.unknowns,
          }),
        });
        const payload = await response.json();
        if (!response.ok) throw new Error(payload?.error || "ClearCFO could not generate the AI analysis.");
        if (!cancelled) setAnalysis(normalizeAnalysis(payload.analysis as AIAnalysis));
      } catch (err) {
    ...
    <div className="px-5 py-8 sm:px-8 sm:py-12 lg:py-16">
      <div className="mx-auto w-full max-w-5xl">
        <div className="mb-6 flex items-center justify-between gap-4">
          <a href="/customer/briefing" className="text-sm font-semibold text-blue-600 hover:text-blue-700">← Back to CFO Briefing</a>
          <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold uppercase tracking-wide text-blue-700">AI Analysis</span>
        </div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-7 shadow-[0_25px_80px_-35px_rgba(15,23,42,0.35)] sm:p-10">
          ...
          <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">What should management do next?</h1>
          <p className="mt-2 text-sm text-slate-500">{data.companyName} · Analysis based on the financial signals currently loaded in ClearCFO.</p>

          {loading && <div className="mt-8 rounded-2xl bg-slate-50 p-6 text-sm text-slate-600">Analyzing financial signals…</div>}
          {error && <div className="mt-8 rounded-2xl border border-red-200 bg-red-50 p-5 text-sm leading-6 text-red-700">{error}</div>}

          {analysis && (
            <div className="mt-8 space-y-5">
              <div className="rounded-2xl bg-slate-50 p-6">
                <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Executive summary</p>
                <p className="mt-2 text-sm leading-6 text-slate-700">{analysis.executiveSummary}</p>
              </div>

              <div className="grid gap-5 sm:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 p-6">
                  <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Primary driver</p>
                  <p className="mt-2 text-sm leading-6 text-slate-700">{analysis.primaryDriver}</p>
                </div>
                <div className="rounded-2xl border border-slate-200 p-6">
                  <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Why it matters</p>
              ...
              <div className="rounded-2xl border border-slate-200 p-6">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-700">{analysis.managementQuestion}</p>
              </div>
              ...
              {analysis.actions?.length > 0 && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Priority actions</p>
                  <div className="mt-3 space-y-3">
