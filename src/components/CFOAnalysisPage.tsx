...
const ANALYSIS_CACHE_KEY = "clearcfo_ai_analysis_cache_v1";
...
function analysisFingerprint(input: BriefingData) {
  return JSON.stringify({ companyName: input.companyName, revenue: input.revenue, revenueChange: input.revenueChange, grossMargin: input.grossMargin, marginChange: input.marginChange, cash: input.cash, cashChange: input.cashChange, inventory: input.inventory, inventoryChange: input.inventoryChange, alerts: input.alerts, drivers: input.drivers, relationships: input.relationships, detailDrivers: input.detailDrivers, recommendation: input.recommendation, health: input.health, confidence: input.confidence, trend: input.trend.slice(-12), periods: input.periods.slice(-12), trendInsights: input.trendInsights, unknowns: input.unknowns });
}
    ...
    <div className="px-5 py-8 sm:px-8 sm:py-12 lg:py-16"><div className="mx-auto w-full max-w-5xl">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-7 shadow-[0_25px_80px_-35px_rgba(15,23,42,0.35)] sm:p-10">
        ...
        {analysis && <div className="mt-8 space-y-5">
          ...
          <div className="rounded-2xl border border-blue-200 bg-blue-50/50 p-6"><p className="text-xs font-bold uppercase tracking-wide text-blue-600">Recommended action</p><div className="mt-3 space-y-3">{splitRecommendedActions(analysis.recommendedAction).map((action, index) => <div key={`${action}-${index}`} className="flex gap-3 text-sm leading-6 text-slate-700"><span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-blue-100 text-xs font-bold text-blue-700">{index + 1}</span><p>{action.replace(/^\d+\.\s*/, "")}</p></div>)}</div></div>
