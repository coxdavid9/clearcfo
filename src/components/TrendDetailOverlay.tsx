...
function readMetricFromCard(target: HTMLElement): SelectedTrend | null {
  ...
  const text = card.textContent ?? "";
  const latest = text.match(/Latest\s+([^\s]+(?:\s+[\d,]+)?)/)?.[1] ?? "See briefing";
  const prior = text.match(/Prior\s+([^\s]+(?:\s+[\d,]+)?)/)?.[1] ?? "See briefing";
    ...
    <div className="fixed inset-0 z-[60] overflow-y-auto bg-slate-50">
      <div className="sticky top-0 z-10 border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur sm:px-8">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          <button type="button" onClick={() => setSelected(null)} className="text-sm font-semibold text-blue-600 hover:text-blue-800">
            ← Back to Financial Trends
          </button>
      ...
      <main className="mx-auto max-w-6xl px-5 py-7 sm:px-8 sm:py-10">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-600">Financial detail</p>
              <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">{selected.metric}</h1>
              <p className="mt-2 text-sm text-slate-500">A deeper look at the latest result and the trend behind it.</p>
            </div>
            <div className="sm:text-right">
              <p className="text-3xl font-bold text-slate-900">{selected.latest}</p>
              <p className={`mt-1 text-sm font-bold ${selected.change.startsWith("-") ? "text-red-600" : "text-emerald-600"}`}>
                {selected.change} vs. prior · {selected.direction}
              </p>
            </div>
          </div>
          ...
          <div className="mt-7 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Latest period</p>
              <p className="mt-2 text-xl font-bold text-slate-900">{selected.latest}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Prior period</p>
              <p className="mt-2 text-xl font-bold text-slate-900">{selected.prior}</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Direction</p>
              <p className="mt-2 text-xl font-bold text-slate-900">{selected.direction}</p>
            </div>
          </div>

          <div className="mt-6 rounded-2xl border border-blue-100 bg-blue-50/50 p-5">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-blue-700">What ClearCFO sees</p>
            <p className="mt-2 text-base font-semibold leading-6 text-slate-900">
              {selected.metric === "Inventory" ? "Inventory is growing faster than revenue, increasing working-capital pressure." :
               selected.metric === "Cash" ? "Cash movement should be evaluated alongside working capital and operating performance." :
               selected.metric === "Operating Expenses" ? "Operating expense growth is an important test of operating leverage." :
               "Revenue is the starting point for judging whether margins, expenses, and working capital are keeping pace."}
            </p>
          </div>

          <div className="mt-5 grid gap-5 lg:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Management question</p>
              <p className="mt-2 text-sm leading-6 text-slate-700">{takeaway}</p>
            </div>
            <div className="rounded-2xl border border-amber-200 bg-amber-50/60 p-5">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-600">What ClearCFO does not know yet</p>
              <p className="mt-2 text-sm leading-6 text-slate-600">{unknown}</p>
            </div>
          </div>

          <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">Recommended next step</p>
            <p className="mt-2 text-base font-semibold leading-6 text-slate-900">{takeaway}</p>
          </div>
        </div>
      </main>
    </div>
