    ...
    <section className="mx-auto mt-8 w-full max-w-6xl px-5 pb-8 sm:px-8 sm:pb-12 lg:pb-16" aria-labelledby="ai-analysis-heading">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_25px_80px_-35px_rgba(15,23,42,0.25)] sm:p-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          ...
          {!analysis && (
            <button type="button" onClick={() => void generateAnalysis()} disabled={loading} className="self-start rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">
              {loading ? "Analyzing…" : "View AI Analysis"}
            </button>
          )}
          {analysis && (
            <button type="button" onClick={() => setExpanded((current) => !current)} className="self-start rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition-all duration-200 hover:border-blue-300 hover:bg-slate-50">
        ...
        {error && (
          <div className="mt-6 rounded-2xl border border-red-200 bg-red-50 p-5 text-sm leading-6 text-red-700">
            <p>{error}</p>
            <button type="button" onClick={() => void generateAnalysis()} disabled={loading} className="mt-3 font-semibold text-red-800 underline underline-offset-2 disabled:opacity-50">Try again</button>
          </div>
        )}
        ...
        {analysis && expanded && (
          <div className="mt-6 space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 p-5"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Primary driver</p><p className="mt-2 text-sm leading-6 text-slate-700">{analysis.primaryDriver}</p></div>
              <div className="rounded-2xl border border-slate-200 p-5"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Why it matters</p><p className="mt-2 text-sm leading-6 text-slate-700">{analysis.whyItMatters}</p></div>
            </div>
