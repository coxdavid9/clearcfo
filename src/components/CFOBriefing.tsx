...
export default function CFOBriefing() {
  ...
  const metrics = [
    ...
    { key: "cash" as const, label: "Cash Position", value: currency.format(data.liveCash ?? data.cash), change: data.liveCashChangePct ?? data.cashChange, tone: (data.liveCashChange ?? 0) > 0 ? "text-emerald-600" : (data.liveCashChange ?? 0) < 0 ? "text-red-600" : "text-amber-600", signal: (data.liveCashChange ?? 0) > 0 ? "bg-emerald-500" : (data.liveCashChange ?? 0) < 0 ? "bg-red-500" : "bg-amber-400", cashDelta: data.liveCashChange, cashAsOfDate: data.cashAsOfDate },
              ...
              {metrics.map((metric) => {
                ...
                return (
                  <button key={metric.key} type="button" onClick={() => setActiveMetricKey(metric.key)} aria-pressed={isSelected} className={`relative min-h-[132px] w-full rounded-2xl border p-5 text-left shadow-sm transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600/30 ${isSelected ? "border-blue-400 bg-blue-50/70 shadow-md shadow-blue-900/10" : "border-slate-200 bg-white opacity-70 hover:opacity-100 hover:border-blue-200 hover:shadow-md"}`}>
                    ...
                    {metric.key === "cash" && metric.cashAsOfDate && <p className="mt-1 text-[10px] font-medium text-slate-400">{metric.cashAsOfDate}</p>}
                    <div className="mt-1 flex flex-wrap items-center gap-1.5 text-xs font-semibold sm:text-sm">
                      <span className={metric.tone}>{metric.key === "cash" && Number.isFinite(metric.cashDelta) ? `${metric.cashDelta >= 0 ? "+" : "−"}${currency.format(Math.abs(metric.cashDelta))}` : displayChange(metric.change, metric.key, metric.key === "inventory" ? data.inventory : undefined)}</span>
                      <span className="font-normal text-slate-400">{metric.key === "cash" ? `vs. ${data.cashAsOfDate ? (() => { const d = new Date(data.cashAsOfDate); return d.toString() !== "Invalid Date" ? d.toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "prior month-end"; })() : "prior month-end"}` : "vs. prior period"}</span>
                    </div>
