      ...
      <div className="mx-auto w-full max-w-6xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_25px_80px_-35px_rgba(15,23,42,0.35)]">
        ...
        <div className="p-7 sm:p-10">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { key: "revenue" as const, label: "Revenue", value: currency.format(data.revenue), change: data.revenueChange, detail: "Current period revenue compared with the prior period." },
              { key: "margin" as const, label: "Gross Margin", value: formatPercentValue(data.grossMargin), change: data.marginChange, detail: "Gross profit as a percentage of revenue." },
              { key: "cash" as const, label: "Cash Position", value: currency.format(data.cash), change: data.cashChange, detail: "Cash and cash-equivalent accounts reported by QuickBooks." },
              { key: "inventory" as const, label: "Inventory", value: currency.format(data.inventory), change: data.inventoryChange, detail: "Inventory asset balance from QuickBooks." },
            ].map((metric) => (
              <button key={metric.key} type="button" onClick={() => toggleMetric(metric.key)} className={`rounded-2xl border p-5 text-left transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md ${expandedMetric === metric.key ? "border-blue-300 bg-blue-50/50 shadow-sm" : "border-slate-200 bg-white"}`}>
                ...
                <p className={`mt-2 text-xs font-semibold ${metric.change >= 0 ? "text-emerald-600" : "text-red-600"}`}>{metric.change >= 0 ? "+" : ""}{formatPercentValue(metric.change)} vs prior period</p>
                {expandedMetric === metric.key && <p className="mt-3 border-t border-slate-200 pt-3 text-xs leading-5 text-slate-600">{metric.detail}</p>}
              </button>
            ))}
          </div>
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-[1.4fr_0.6fr]">
            <div className="rounded-2xl border border-slate-200 bg-white p-6">
              <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Revenue trend</p><p className="mt-1 text-sm text-slate-500">Monthly revenue over the available reporting period.</p></div><span className={`text-sm font-bold ${trendChange >= 0 ? "text-emerald-600" : "text-red-600"}`}>{trendChange >= 0 ? "+" : ""}{formatPercentValue(trendChange)}</span></div>
              <div className="mt-6 flex h-48 items-end gap-2 border-b border-slate-200 pb-2">{data.trend.map((value, index) => <div key={`${data.periods[index] || index}-${index}`} className="flex h-full flex-1 items-end"><div className="w-full rounded-t-md bg-blue-500/80" style={{ height: `${trendHeights[index] || 18}%` }} title={`${data.periods[index] || "Period"}: ${currency.format(value)}`} /></div>)}</div>
              <div className="mt-3 flex justify-between text-[10px] font-medium text-slate-400">{data.periods.map((period, index) => <span key={`${period}-${index}`} className={index % Math.max(1, Math.ceil(data.periods.length / 4)) === 0 || index === data.periods.length - 1 ? "" : "hidden sm:inline"}>{period}</span>)}</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Decision focus</p><h3 className="mt-2 text-lg font-bold text-slate-900">{data.recommendation}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{data.drivers[0]?.observation || "Review the latest financial signals and determine the most important management action."}</p></div>
          </div>
          ...
          <div className="mt-10 rounded-2xl border border-slate-200 bg-white p-6 sm:p-7">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h3 className="mt-2 text-xl font-bold text-slate-900">Turn the signals into a decision.</h3>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{deterministicAnalysis.summary}</p>
              </div>
              <button type="button" onClick={() => setShowAnalysis((current) => !current)} className="self-start rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-blue-700">{showAnalysis ? "Hide AI Analysis" : "View AI Analysis"}</button>
            </div>
            {showAnalysis && <div className="mt-6 border-t border-slate-200 pt-6">
              {aiLoading && <div className="rounded-2xl bg-slate-50 p-5 text-sm text-slate-600">Analyzing financial signals…</div>}
              {aiError && <div className="mt-5 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{aiError}</div>}
              {aiAnalysis && <div className="space-y-5">
                <div className="rounded-2xl bg-slate-50 p-5"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Executive summary</p><p className="mt-2 text-sm leading-6 text-slate-700">{aiAnalysis.executiveSummary}</p></div>
                <div className="grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-slate-200 p-5"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Primary driver</p><p className="mt-2 text-sm leading-6 text-slate-700">{aiAnalysis.primaryDriver}</p></div><div className="rounded-2xl border border-slate-200 p-5"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Why it matters</p><p className="mt-2 text-sm leading-6 text-slate-700">{aiAnalysis.whyItMatters}</p></div></div>
                <div className="rounded-2xl border border-slate-200 p-5"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Management question</p><p className="mt-2 text-sm leading-6 text-slate-700">{aiAnalysis.managementQuestion}</p></div>
                <div className="rounded-2xl border border-blue-200 bg-blue-50/40 p-5"><p className="text-xs font-bold uppercase tracking-wide text-blue-600">Recommended action</p><p className="mt-2 text-sm leading-6 text-slate-700">{aiAnalysis.recommendedAction}</p></div>
                {aiAnalysis.actions?.length > 0 && <div><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Priority actions</p><div className="mt-3 space-y-3">{aiAnalysis.actions.map((action, index) => <div key={`${action.title}-${index}`} className="rounded-2xl border border-slate-200 p-5"><div className="flex items-center justify-between gap-3"><p className="font-semibold text-slate-900">{action.title}</p><span className="text-xs font-semibold uppercase tracking-wide text-slate-400">{action.priority}</span></div><p className="mt-2 text-sm leading-6 text-slate-600">{action.rationale}</p></div>)}</div></div>}
              </div>}
            </div>}
          </div>
        </div>
