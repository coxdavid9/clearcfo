...
export default function Navbar({ onNavigate, onLogin, loginHref, loginLabel = "Log In", signupHref, signupLabel = "Sign Up", profileHref, sessionAware = false }: NavbarProps) {
  ...
  const authenticatedAuthControl = (
    <div className="hidden items-center gap-3 sm:flex">
      ...
      <a href="/customer" className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-blue-200 hover:text-blue-600 hover:shadow-md sm:px-5">CFO Briefing</a>
  ...
  const mobileAuthControl = sessionAware && authenticated ? (
    <>
      ...
      <a href="/customer" onClick={() => setMenuOpen(false)} className="border-b border-slate-100 px-1 py-4 text-left text-base font-medium text-slate-700 transition-colors hover:text-blue-600">CFO Briefing</a>
