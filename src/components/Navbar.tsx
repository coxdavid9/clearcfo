...
export default function Navbar({ onNavigate, onLogin, loginHref, loginLabel = "Log In", signupHref, signupLabel = "Sign Up", profileHref, sessionAware = false }: NavbarProps) {
  ...
  const authenticatedAuthControl = (
    <div className="hidden items-center gap-3 sm:flex">
      <a href="/customer" className="px-1 py-2 text-sm font-semibold text-blue-600 transition-colors hover:text-blue-700">Dashboard</a>
      <a href="/customer/briefing" className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-blue-200 hover:text-blue-600 hover:shadow-md sm:px-5">CFO Briefing</a>
  ...
  const mobileAuthControl = sessionAware && authenticated ? (
    <>
      <a href="/customer" onClick={() => setMenuOpen(false)} className="border-b border-slate-100 px-1 py-4 text-left text-base font-semibold text-blue-600">Dashboard</a>
      <a href="/customer/briefing" onClick={() => setMenuOpen(false)} className="border-b border-slate-100 px-1 py-4 text-left text-base font-medium text-slate-700 transition-colors hover:text-blue-600">CFO Briefing</a>
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-white/90 backdrop-blur-md">
      ...
      {menuOpen && <div className="border-t border-slate-200 bg-white px-5 pb-5 pt-3 shadow-lg md:hidden"><div className="mx-auto flex max-w-7xl flex-col">{navItems.map(([label, href]) => onNavigate ? <button key={href} type="button" onClick={() => navigate(href)} className="border-b border-slate-100 px-1 py-4 text-left text-base font-semibold text-slate-700 transition-colors hover:text-blue-600">{label}</button> : <a key={href} href={`/${href}`} onClick={() => setMenuOpen(false)} className="border-b border-slate-100 px-1 py-4 text-left text-base font-semibold text-slate-700 transition-colors hover:text-blue-600">{label}</a>)}{mobileAuthControl}</div></div>}
    </header>
  );
}
