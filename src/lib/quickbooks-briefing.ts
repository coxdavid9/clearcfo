...
function valuesFromCells(cells: any[] | undefined, periodCount: number): number[] {
  // QuickBooks reports include a trailing Total column. The dashboard must use
  // only the requested monthly periods; including Total shifts every series.
...
function reportPeriods(report: any): string[] {
  return (report?.Columns?.Column || []).slice(1).map((column: any) => String(column?.ColTitle || "").trim()).filter((title: string) => title && !/^total$/i.test(title));
}
...
function changePercent(current: number, previous: number): number {
  // No percentage is meaningful when the comparison period is zero.
...
function buildDrivers(revenueChange: number, marginChange: number, cashChange: number, inventoryChange: number, expenseChange: number): FinancialDriver[] {
  ...
  if (expenseChange > revenueChange + 2) drivers.push({ id: "opex-growth", category: "Operating Expense", title: "Operating expenses are rising faster than revenue", observation: `Operating expenses changed ${formatPercent(expenseChange)} while revenue changed ${formatPercent(revenueChange)}.`, evidence: [`Operating expense change: ${formatPercent(expenseChange)}`, `Revenue change: ${formatPercent(revenueChange)}`], direction: "up", severity: expenseChange > 20 ? "High" : "Medium", impact: Math.min(10, Math.max(1, Math.round(Math.abs(expenseChange - revenueChange) / 5))), confidence: 0.9, managementQuestion: "Which expense categories are driving the increase, and which are controllable or temporary?" });
  if (cashChange < -5) drivers.push({ id: "cash-pressure", category: "Cash", title: "Cash is under pressure", observation: `Cash declined ${formatPercent(Math.abs(cashChange))} from the prior period.`, evidence: [`Cash change: ${formatPercent(cashChange)}`], direction: "down", severity: cashChange < -15 ? "High" : "Medium", impact: 5, confidence: 0.94, managementQuestion: "What near-term cash commitments could create additional pressure?" });
  if (inventoryChange > revenueChange + 2) drivers.push({ id: "inventory-growth", category: "Inventory", title: "Inventory is outpacing revenue", observation: `Inventory changed ${formatPercent(inventoryChange)}, ahead of revenue at ${formatPercent(revenueChange)}.`, evidence: [`Inventory change: ${formatPercent(inventoryChange)}`, `Revenue change: ${formatPercent(revenueChange)}`], direction: "up", severity: "Medium", impact: 3, confidence: 0.9, managementQuestion: "What is driving the inventory build, and how quickly can it be converted to sales?" });
  if (marginChange < -2) drivers.push({ id: "margin-pressure", category: "Margin", title: "Gross margin has weakened", observation: `Gross margin changed ${formatPercent(marginChange)} from the prior period.`, evidence: [`Margin change: ${formatPercent(marginChange)}`], direction: "down", severity: marginChange < -5 ? "High" : "Medium", impact: 4, confidence: 0.88, managementQuestion: "Is the margin change coming from pricing, product mix, or direct costs?" });
  return drivers;
}
...
export function buildQuickBooksBriefing(profitAndLoss: any, balanceSheet: any, companyName: string | null): BriefingData {
  ...
  // Prefer QuickBooks' own section totals. Do not reconstruct totals from
  // child accounts when the authoritative section summary is present.
  ...
  const cash = balancePeriods.length ? pickSeries(balanceRows, [/^cashandcashequivalents$/, /^cash$/, /^cashandbank$/], [/^total cash and cash equivalents$/, /^cash and cash equivalents$/, /^total cash$/], balancePeriods.length) : null;
  ...
  // P&L and Balance Sheet are now both parsed to their actual monthly columns.
  // Do not shift or pad with the Balance Sheet's trailing Total column.
  const cashAligned = cashSeries ? cashSeries.slice(-pnlPeriods.length) : [];
  const inventoryAligned = inventory ? inventory.slice(-pnlPeriods.length) : [];
  const current = pnlPeriods.length - 1;
  ...
  const currentExpense = expenses[current] || 0;
  const previousExpense = expenses[previous] || 0;
  ...
  const trendSeries: Series[] = [
    ...
    { name: "Gross Margin", values: pnlPeriods.slice(-12).map((_, index) => { const sourceIndex = Math.max(0, pnlPeriods.length - Math.min(12, pnlPeriods.length)) + index; return revenue[sourceIndex] ? (grossProfit[sourceIndex] / revenue[sourceIndex]) * 100 : 0; }), periods: pnlPeriods.slice(-12) },
    { name: "Operating Expenses", values: expenses.slice(-12), periods: pnlPeriods.slice(-12) },
  ...
  if (!revenue.some((value) => value !== 0) && !netIncome?.some((value) => value !== 0)) throw new Error("QuickBooks is connected, but ClearCFO could not identify a non-zero income or net-income series in the returned P&L.");
